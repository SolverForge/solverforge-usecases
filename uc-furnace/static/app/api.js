export function buildApiGuideEndpoints() {
  return [
    { method: 'GET', path: '/demo-data/STANDARD', description: 'Fetch the published standard demo payload', curl: buildCurlCommand('GET', '/demo-data/STANDARD') },
    { method: 'POST', path: '/jobs', description: 'Create a retained solving job from the published Plan DTO', curl: buildCurlCommand('POST', '/jobs', { json: true, data: '@plan.json' }) },
    ...
    { method: 'GET', path: '/jobs/{id}/analysis/{constraint_name}', description: 'Inspect one constraint in detail', curl: buildCurlCommand('GET', '/jobs/{id}/analysis/unassignedOrders', { quoteUrl: true }) },
    { method: 'POST', path: '/jobs/{id}/pause', description: 'Pause a retained solving job', curl: buildCurlCommand('POST', '/jobs/{id}/pause') },
