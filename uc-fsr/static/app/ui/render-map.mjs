/* render-map.mjs - Leaflet map rendering for the Bergamo FSR demo */

import { assignedVisitSet, iconForVisit, legFor, locationLat, locationLng } from '../models/utils.mjs';
...
export function createMapRenderer(options) {
  ...
  function renderMap(plan, routeGeometry) {
    ...
    routes.forEach(function (route) {
      ...
      if (!start) return;
      fitPoints.push([locationLat(start), locationLng(start)]);
    ...
    visits.forEach(function (visit, idx) {
      ...
      if (!location) return;
      fitPoints.push([locationLat(location), locationLng(location)]);
  ...
  function stopLocationKey(location) {
    return [
      locationLat(location).toFixed(6),
      locationLng(location).toFixed(6),
    ].join(',');
  }
