...
use solverforge::{
    HardSoftScore, SolverEventMetadata, SolverLifecycleState, SolverManagerError, SolverSnapshot,
...
#[derive(Serialize)]
#[serde(rename_all = "camelCase")]
pub(super) struct JobEventPayload {
    id: String,
    job_id: String,
...
pub(super) fn parse_job_id(id: &str) -> Result<usize, SolverManagerError> {
    id.parse::<usize>()
...
pub(super) fn status_event_payload(
    ...
    serialize_payload(JobEventPayload {
        ...
        telemetry: telemetry_payload(status.telemetry.clone()),
...
pub(super) fn event_payload(
    ...
    serialize_payload(JobEventPayload {
        ...
        telemetry: telemetry_payload(metadata.telemetry.clone()),
...
pub(super) fn serialize_payload(payload: JobEventPayload) -> String {
    serde_json::to_string(&payload).expect("failed to serialize solver lifecycle payload")
}

pub(super) fn telemetry_payload(telemetry: SolverTelemetry) -> TelemetryPayload {
    TelemetryPayload {
...
pub(super) fn lifecycle_state_label(state: SolverLifecycleState) -> &'static str {
    match state {
...
pub(super) fn terminal_reason_label(reason: SolverTerminalReason) -> &'static str {
    match reason {
...
pub(super) fn whole_units_per_second(count: u64, elapsed: Duration) -> u64 {
...
pub(super) fn derive_acceptance_rate(moves_accepted: u64, moves_evaluated: u64) -> f64 {
