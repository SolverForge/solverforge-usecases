...
use solverforge_maps::{BoundingBox, Coord, NetworkConfig, RoadNetwork, RoutingError, UNREACHABLE};

use crate::domain::Plan;

use super::helpers::delivery_coords;
...
pub async fn prepare_plan(plan: &mut Plan) -> Result<(), RoutingError> {
    ...
    let depot_routing = DepotRoutingData::road_network(plan, &delivery_coords).await?;
    ...
    for (vehicle_idx, vehicle) in plan.vehicles.iter_mut().enumerate() {
        ...
        vehicle.prepared_routing = Some(PreparedVehicleRouting {
            ...
            capacity: vehicle.capacity as i64,
            demands: delivery_demands.clone(),
            distance_matrix: depot_routing.delivery_distance_meters.clone(),
            time_windows: delivery_time_windows.clone(),
            service_durations: delivery_service_durations.clone(),
...
struct DepotRoutingData {
    ...
    delivery_distance_meters: Vec<Vec<i64>>,
    ...
    async fn road_network(plan: &Plan, delivery_coords: &[Coord]) -> Result<Self, RoutingError> {
        ...
        if all_coords.is_empty() {
            return Ok(Self {
                ...
                delivery_distance_meters: Vec::new(),
        ...
        let mut delivery_distance_meters = vec![vec![0_i64; delivery_count]; delivery_count];
        for (i, row) in delivery_travel_times.iter_mut().enumerate() {
            for (j, cell) in row.iter_mut().enumerate() {
                *cell = matrix.get(i, j).unwrap_or(UNREACHABLE);
            }
        }
        for (i, row) in delivery_distance_meters.iter_mut().enumerate() {
            for (j, cell) in row.iter_mut().enumerate() {
                *cell = matrix.distance_meters(i, j).unwrap_or(UNREACHABLE);
            }
        }
        ...
        for vehicle_idx in 0..plan.vehicles.len() {
            ...
            for delivery_idx in 0..delivery_count {
                ...
                outbound_meters.push(
                    matrix
                        .distance_meters(depot_idx, delivery_idx)
                        .unwrap_or(UNREACHABLE),
                );
                inbound_meters.push(
                    matrix
                        .distance_meters(delivery_idx, depot_idx)
                        .unwrap_or(UNREACHABLE),
                );
            }
